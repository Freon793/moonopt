...
> `M5`（整数规划）第一轮已落地：分支定界骨架 + **每个松弛都由独立校验器复核**。这条复核在第一轮抓到
> 三例被拒证书，第二轮查明**三个节点其实都可行**（外部求解器 HiGHS 逐一确认），也就是内核的
> `Infeasible` **判定**错了；其中两个成因（Phase I 的容差边际判定、对偶单纯形的越界结论）已修，
> 剩下 `noswot` 一个节点上的对偶符号差 2e-6 仍列为**已知缺陷**（见 CHANGELOG 与 `docs/roadmap.md`）
