...
### Known defect — `noswot` 的一个节点上对偶符号差约 2e-6（第三例，未修）

- `noswot` 节点 182（深度 16）被拒的原因是 `dual signs failed: 2.14e-6`：**最优性**证书里一个应当非负的
  乘子略微为负（量级约 1e-6，模型价格量级约 43，相对约 5e-8）。它既可能是内核的对偶值在退化下算错了一点，
  也可能是校验器对这个家族仍然过严 —— 这一轮没有定论，判定它需要把该节点的模型与证书导出、再让外部求解器
  确认那份"最优解"是否真的最优。行为仍然安全（搜索停下报 `Unverified`），但 `noswot` 暂时不能由分支定界求解，
  `bench/report-mip.ps1` 也继续按纪律拒绝写 MIP 报告（32 个实例里只有它一个 `unverified`）。
