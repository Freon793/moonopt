...
**改法**：9 个包各新增 `extends.mbt`（与 core 的惯例一致，迁移集中可审计），共 **28 个类型、48 条声明** ——
`Debug::{to_repr}` 28 条，`Eq::{equal}` 与 `Eq::{not_equal}` 各 10 条。
**Eq 必须两个方法各写一条**：第一次只写了 `equal`，CI 的注解立刻指出漏的是 `not_equal`
（这正是"先让 CI 自己说明原因"的价值所在）。每条都带 **`#doc(hidden)`**：它们是 trait 自己的方法、
不是新的承诺，`moon info` 实测 **`.mbti` 一行未变**，行为按构造不变（原来隐式发生的提升，现在写出来）。
同一份"非 priv 派生类型 × trait 方法"集合与已声明集合做了**逐项交叉核对：零缺失、零多余**。
需要该 trait 的包在 `moon.pkg` 里补了 `moonbitlang/core/debug`（标准库，不是第三方依赖）。

**两处值得长期记住的事实**：① `setup-moonbit` 默认装 `latest`，所以**编译器升级能让 CI 在没有任何代码改动时变红**；
② 读 CI 报错的可行路径是 **annotations（匿名可读）**，不是 job 日志（要鉴权）。
