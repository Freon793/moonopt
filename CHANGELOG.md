...
### Known defect — 内核 Phase I 的不可行证书不是总成立（M5 后续必须解决）

- 内核报 `Infeasible` 时给出的是 **Phase I 最优点的对偶解**（`state.model_duals()`），
  代码注释里假设"Phase I 的对偶就是 Farkas 射线"。本轮在分支定界里跑到 200–300 个节点时，
  这个假设在三个实例上被 `verify` 拒绝（**结论本身是对的，证书不成立**）：
  - `misc07`：节点 58（深度 16）`farkas margin failed: -4.3e-13`；
  - `blend2`：节点 71（深度 13）`farkas margin failed: -20800`；
  - `noswot`：节点 167（深度 12）`ray signs failed: 0.9538`。
  行为是**安全的**（搜索停下并报 `Unverified`，绝不继续用这份证书），但三个实例因此无法求解，
  `bench/report-mip.ps1` 也按纪律拒绝写报告。
  已知的两个成因方向：① Phase I 的双重可行性只保证"人工列方向"上的符号，与模型行乘子的符号约定
  并非总一致；② 退化微扰（`degeneracy_perturbation`）让 Phase I 证的是**微扰后**系统的不可行性，
  而校验器判的是**未微扰**模型 —— `-4.3e-13` 这种量级正是这一项的指纹。
  修法方向是明确的：要么从 Phase I 的状态导出真正满足符号约定的 Farkas 射线，
  要么把不可行判定交给对偶单纯形那条已经把射线做成"基逆的某一行"的路径（`dual.mbt`）。
