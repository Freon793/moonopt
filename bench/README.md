...
它的边界现在是 **Phase II 的每次枢轴增益**：内核目标值在 20000 / 60000 / 250000 次枢轴时
是 165.115 / 164.157 / 160.353（约 2e-5/枢轴），按该速度远不可能收敛。
...
- `solve-report.md`：求解报告。记录模式（是否 LP 松弛）、行数上限、工具链版本与提交哈希，
  ...
  **`refused` 归零是稀疏 LU 的直接结果**：内核不再持有稠密基逆，`30n20b8`（presolve 后 11591 行）
  16 秒求到最优并进入对拍表；`fast0507`（内核 489 行）能跑到 Phase II，
  但它离收敛还差好几个数量级 —— 对这类实例限制既不是内存也不是停滞，而是
  ...
  `fast0507` 是唯一真正超出范围的实例 —— 它的 Phase I 人工和在 1200 / 6000 次枢轴时
  是约 178 / 17.4（从 489 起单调下降），既不是停滞也不是循环（6000 次枢轴里
  零步长枢轴占 2.4%，最长连续无改进 6 次），Phase I 本身在 45000 次枢轴内跑完，
  之后 Phase II 才是真正的墙。这份报告此前把它的目标列写成 `13`，
  ...
  而是把上界当界用（内核行 63490 → 489，已落地），剩下的墙是 Phase II 的每次枢轴增益，
  见 `docs/roadmap.md`。
