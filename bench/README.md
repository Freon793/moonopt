...
它的边界从"内存"变成了"时间"（Phase I 在 500 次迭代 / 61 秒内未收敛）。
...
- `solve-report.md`：求解报告。记录模式（是否 LP 松弛）、行数上限、工具链版本与提交哈希，
  ...
  **`refused` 归零是稀疏 LU 的直接结果**：内核不再持有稠密基逆，`30n20b8`（presolve 后 11591 行）
  16 秒求到最优并进入对拍表；`fast0507`（63490 行）不再被拒绝、能跑（500 次迭代 61 秒），
  但 Phase I 尚未收敛 —— 对这类实例限制已从内存变成时间。
  报告里另外 5 个 `iteration-limit`（`danoint` / `mod010` / `bienst1` / `bienst2` / `fast0507`）
  是**本轮核实用 1200 作迭代上限**造成的：前四个在默认 20000 上限下可解，
  `fast0507` 是唯一真正卡住的实例 —— 而且探针显示它不是"慢"而是**停滞**：
  500 次与 4000 次迭代（61 s 与 267 s）下 Phase I 的人工和都是 13，一点没动，
  因为防停滞切换到 Bland 的判据是 `2m+50` 次无改进，在 63490 行上等于永不触发。
  这是稀疏 LU 之后暴露出来的下一道墙（定价与停滞判据），已记入 roadmap。
