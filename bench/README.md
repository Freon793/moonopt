...
- `solve-report.md`：求解报告。记录模式（是否 LP 松弛）、行数上限、**枢轴上限**、工具链版本与提交哈希，
  ...
  **枢轴上限是基准口径的一部分**（报告头部写明，且 `pivots` 列等于上限的实例就是在那里停下的）：
  ...
  当前这份由 `-Relax -MaxRows 1000 -MaxIterations 1200 -Presolve` 生成，用时 111 秒：
  **32 个实例全覆盖、18 最优、11 因行数上限跳过、`refused` 归零、0 数值失败**，
  21 个实例经过化简、18 个重建解全部通过三项检查、0 次需要 Bland 恢复。
  同一条命令把上限放到脚本默认的 **20000**，同一份清单实测 **20 个求到最优 / 11 跳过 / 1 到上限
  （`fast0507`）、总枢轴 12 336、373 秒**（`danoint` 1907 枢轴、`mod010` 3455 枢轴）—— 那正是历史上
  "20 最优"那份报告的实测口径（`747de9e`，它的 `fast0507` 记着 20000 个枢轴；当时 `mod010` 要 12 827 个）。
  ...
  **用 1200 作枢轴上限**造成的，不是内核的限制：`danoint` 1907 枢轴与 `mod010` 3455 枢轴
  都在默认上限 20000 内求到最优。`mod010` 的枢轴数此前被防停滞兜底的**闩死**抬高了 4.5 倍
  （15 527 → 3455），该闩死已在本轮解除（目标重新有改进即解除；见 `CHANGELOG.md`）——
  它**没有改变这份 1200 上限报告的任何一格**，改变的是默认预算下要花多少枢轴。
  这份报告此前把 `fast0507` 的目标列写成 `13`，
