...
$build += "- Mode: $(if ($Relax) { "LP relaxation (integer columns treated as continuous)" } else { "as-is (integer models are refused by the linear kernel)" }), row limit $MaxRows, presolve $(if ($Presolve) { "on" } else { "off" })."
