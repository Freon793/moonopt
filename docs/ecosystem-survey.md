...
| 卡点 | 已知做法 | 本项目这一轮的状态 |
| --- | --- | --- |
| Farkas 射线构造（队列 4） | 从 Phase I 状态显式导出满足乘子符号约定的射线；或从对偶单纯形状态取对偶射线 | **第二十三轮已落地守卫**：内核在发证书前按必需条件自检（人工和 `Σ a` 必须 ≥ 它要吸收的违反量 `max v`），不成立就不发证书、报 `NumericalFailure`；"更一般地显式导出射线"仍未做，但它已不再是阻塞项 |
