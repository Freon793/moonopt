# 与生态中既有实现的事实性对照（核对于 2026-09-14）

本文件记录选题查重的**方法与原始证据**，供项目维护者与评审复核。
结论用于回答“是否与 MoonBit 生态中已有项目高度重复”。
...
**与本项目的关系**：能力边界不同。它不能表达等式/`≥` 约束、变量上下界、
自由变量与最小化（需使用者自行变换），也没有模型文件输入、对偶、presolve 与证书。
本项目不与它竞争“实现存在与否”，而是补齐上述工程化与可验证性要素，并可为其提供
“模型 → 求解内核”的对接路径。
...
**与本项目的关系**：它的建模/标准化思路有价值，本项目将提供**适配层**把它的模型喂进
`moonopt` 的求解内核（见路线图 D9），形成协作而非重复。
